...
function saveState() {
  try {
    localStorage.setItem('ultimateclicker_v2', JSON.stringify(state));
  } catch(e) {}
}
...
// ============================================
// DIGITAL BACKGROUND — wrapped in try/catch
// so a canvas failure never blocks the game
// ============================================
  ...
  try {
    ...
    function playChord() {
      ...
      try {
        ...
        b.stop(audioCtx.currentTime + 2.2);
      } catch(e) {}
    }
...
// Uses a single hard-cap timeout that always
// fires — no chance of it stalling forever.
// ============================================
const loadingMessages = [
  'Initializing Systems...',
  'Loading Click Engine...',
  'Calibrating Coins...',
...
function runLoadingScreen(callback) {
  ...
  // Safety: if elements don't exist, just call callback immediately
  if (!bar || !txtEl) {
    callback();
    return;
  }
  ...
  let pct    = 0;
  let msgIdx = 0;
  let fired  = false; // make sure callback only fires once
  ...
  function finish() {
    ...
    // Small delay so the user sees "Ready!" before screen switches
  ...
  const msgInt = setInterval(() => {
    if (msgIdx < loadingMessages.length) {
      txtEl.textContent = loadingMessages[msgIdx++];
    }
  }, 650);

  // Bar fill — fast random increments
  const barInt = setInterval(() => {
    pct += Math.random() * 28 + 18; // adds 18–46% each tick at ~150ms
    if (pct >= 100) {
      pct = 100;
      bar.style.width = '100%';
      finish();
  ...
  // ABSOLUTE HARD CAP: always fires after 4 seconds no matter what
...
function showSetup() {
  ...
  document.getElementById('start-btn').addEventListener('click', () => {
    ...
    if (!name || name.length < 2) {
      showToast('Enter a username (min 2 chars)!');
      return;
    }
    state.username = name;
...
function updateUI() {
  ...
  if (asl) {
    asl.textContent = state.upgrades.autoclicker === 0
      ? 'Not active'
      : '1 click / ' + Math.max(1, 11 - state.upgrades.autoclicker) + 's';
  }
...
function setupExchange() {
  document.getElementById('exchange-btn').addEventListener('click', () => {
    initAudio();
    if (state.clicks < 10) {
      showToast('Need at least 10 clicks! 👆');
      playError();
      return;
    }
...
// ============================================
// IDLE CLICKER — 1 click every 30 seconds
// ============================================
...
function updateQuestProgress() {
  if (!state.quests.q1.claimed) {
    ...
    if (p >= 500 && !state.quests.q1.done) {
      ...
      const b = document.getElementById('q1-claim');
  ...
  if (!state.quests.q2.claimed) {
    ...
    if (p >= 50 && !state.quests.q2.done) {
      ...
      const b = document.getElementById('q2-claim');
      if (b) b.disabled = false;
    }
  }
  if (!state.quests.q3.claimed) {
    ...
    if (p >= 1 && !state.quests.q3.done) {
      ...
      const b = document.getElementById('q3-claim');
      if (b) b.disabled = false;
    }
  }
}
...
function spawnRainItem() {
  ...
  item.addEventListener('click', e => {
    ...
    clicked = true;
    if (isBomb) {
      rainCaught = Math.max(0, rainCaught - 5);
      showToast('💣 Bomb! -5 coins!');
      playError();
    } else {
      rainCaught++;
      playClick();
...
function resetMemory() {
  ...
  document.querySelectorAll('.mem-btn').forEach((tile, i) => {
    const nt = tile.cloneNode(true);
    tile.parentNode.replaceChild(nt, tile);
    nt.addEventListener('click', () => {
      if (!memOk) return;
      playClick();
      nt.classList.add('flash');
...
function nextMemRound() {
  ...
  flashMem(() => {
    memOk = true;
    document.getElementById('memory-instruction').textContent = 'Your turn! Tap in order! 👆';
  });
}
...
function flashMem(cb) {
  ...
  const go = () => {
    ...
    tiles[memSeq[i]].classList.add('flash');
    i++;
    setTimeout(go, 700);
  };
  setTimeout(go, 400);
...
function checkMemIn() {
  ...
  if (memIn[idx] !== memSeq[idx]) {
    if (tiles[memIn[idx]]) {
      tiles[memIn[idx]].classList.add('wrong');
      setTimeout(() => tiles[memIn[idx]].classList.remove('wrong'), 400);
    }
    playError();
    memOk = false;
    const boost     = 1 + (state.upgrades.minigame * 0.25);
  ...
  if (tiles[memIn[idx]]) {
    tiles[memIn[idx]].classList.add('correct');
    setTimeout(() => tiles[memIn[idx]].classList.remove('correct'), 300);
  }
...
function getBotTypes() {
  ...
  if (lvl < 5) {
    ...
    for (let i = 0; i < count; i++) {
      const r = Math.random();
      pool.push(r < 0.33 ? 'slow' : r < 0.66 ? 'medium' : 'fast');
    }
  } else {
    for (let i = 0; i < count; i++) {
      const r = Math.random();
      pool.push(r < 0.2 ? 'slow' : r < 0.5 ? 'medium' : 'fast');
    }
  }
  return pool;
}
...
function generateBots() {
  ...
  const el = document.getElementById('race-bots');
  if (!el) return;
  el.innerHTML = '';
  raceBots.forEach(bot => {
    ...
    card.innerHTML = `
      ...
      <div>
        ...
        <div class="race-bot-speed">Speed: ${lbl}</div>
      </div>`;
    el.appendChild(card);
  });
}
...
function updateRaceScoreboard() {
  const e = document.getElementById('race-you-clicks');
  if (e) e.textContent = racePlayerClicks + ' clicks';
  raceBots.forEach((bot, i) => {
...
function endRace() {
  ...
  if (won) {
    ...
    box.innerHTML = `
      ...
      <p>No auto-clickers — raw tapping only! 💪</p>`;
    playError();
  }
  checkAchievements();
  updateUI();
...
function giveWeeklyRewards() {
  ...
  const rewards = {
    1: { coins:1000, clicks:1000 },
    2: { coins:500,  clicks:500  },
    3: { coins:250,  clicks:250  },
    4: { coins:100,  clicks:0    },
    5: { coins:50,   clicks:0    },
  };
...
function renderLeaderboard(type) {
  ...
  const allNames = realNames.concat(
  ...
  if (saveBtn) {
    saveBtn.addEventListener('click', () => {
      ...
      if (!newName || newName.length < 2) {
        showToast('Username must be at least 2 chars!');
        return;
      }
  ...
  runLoadingScreen(() => {
    try {
      ...
      if (hasSave) {
        showGame();
        setTimeout(() => {
          try { initAudio(); startLofi(); } catch(e) {}
        }, 500);
      } else {
        showSetup();
      }
    } catch(e) {
      // Last resort: if anything fails, force-hide loading and show setup
      const ls = document.getElementById('loading-screen');
